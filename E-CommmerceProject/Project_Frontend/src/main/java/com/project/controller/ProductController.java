...
public class ProductController {
	...
	@RequestMapping(value = "/all/getallproductsforuser")
	public String getAllProductsForUser(Model model ) {
		...
		model.addAttribute("productslist", products);
		return "userhomepage";// logical view name
		// in listofproducts.jsp,access the model attribute "productsList"
	...
	public String updateProduct(@Valid @ModelAttribute(name = "product") Product product, BindingResult result,HttpServletRequest request,Model model) {
		if (result.hasErrors()) {
			model.addAttribute("categories", categoryDao.getAllCategories());
		...
		System.out.println(request.getServletContext().getRealPath("/"));
		//Defining a path
		Path path=Paths.get(request.getServletContext().getRealPath("/")+"/resources/img/"+product.getId()+".png");
		//transfer the image to the file
		if(!img.isEmpty() && img!=null) {
			try {
				img.transferTo(new File(path.toString()));
